...
describe('Etapa 00 — capabilities: stored flags ≠ effective availability', () => {
  it('paymentMode DIGITAL_MP no produce AVAILABLE capability', async () => {
    mocks.getCapabilities.mockResolvedValue({
      ...
      capabilities: {
        digital_payment: { key: 'digital_payment', state: CapabilityState.COMING_SOON, reasonCode: 'DIGITAL_PAYMENTS_UNAVAILABLE', message: expect.any(String) }
      }
    });
  ...
  it('allowSplitBill true no produce AVAILABLE capability', async () => {
    mocks.getCapabilities.mockResolvedValue({
      restaurantId: 'restaurant-a',
      capabilities: {
        split_bill: { key: 'split_bill', state: CapabilityState.COMING_SOON, reasonCode: 'SPLIT_BILL_UNAVAILABLE', message: expect.any(String) }
      }
    });
    const app = await appWithRoutes();
    try {
  ...
  it('reviews enabled without googlePlaceId → MISCONFIGURED', async () => {
    mocks.getCapabilities.mockResolvedValue({
      restaurantId: 'restaurant-a',
      capabilities: {
        reviews: { key: 'reviews', state: CapabilityState.MISCONFIGURED, reasonCode: 'REVIEWS_NO_PLACE_ID', message: expect.any(String) }
      }
    });
    const app = await appWithRoutes();
    try {
