...
describe('E01 — Importación de Menú Idempotente, No Destructiva y con dryRun', () => {
  ...
  it('10. Rechaza flags no booleanos y precios fuera del rango Int antes de mutar', async () => {
    mocks.categoryFindMany.mockResolvedValue([]);
    const app = await createApp();
    try {
      const token = app.jwt.sign({ sub: 'manager-1', role: 'MANAGER', restaurantId: 'restaurant-fauno' });
      const validItem = { category: 'Bebidas', name: 'Agua', price: 1000 };
      const invalidPayloads: Array<Record<string, unknown>> = [
        { replaceExisting: 'false', items: [validItem] },
        { dryRun: 'false', items: [validItem] },
        { items: [{ ...validItem, isAvailable: 'false' }] },
        { items: [{ ...validItem, isFeatured: 'false' }] },
        { items: [{ ...validItem, price: 21_474_836.48 }] },
        { items: [{ ...validItem, price: null }] }
      ];
      ...
      const maxValidPrice = await app.inject({
        method: 'POST',
        url: '/restaurants/restaurant-fauno/menu/import',
        headers: { authorization: `Bearer ${token}` },
        payload: { dryRun: true, items: [{ ...validItem, price: 21_474_836.47 }] }
      });
      expect(maxValidPrice.statusCode).toBe(200);
      expect(maxValidPrice.json().dryRun).toBe(true);
      expect(mocks.categoryCreate).not.toHaveBeenCalled();
      expect(mocks.itemCreate).not.toHaveBeenCalled();
    } finally {
      await app.close();
  ...
  it('11. GET /restaurants/:slugOrId/menu conserva categorías con COMING_SOON pero excluye inactivas normales salvo includeEmpty=true (P2/P3)', async () => {
