...
export class WaitlistService {
  ...
  static async seatGuest(
    ...
    const previousStatus = entryEarly.status as WaitlistStatus;
  ...
  /**
   * @deprecated E07: la reversión atómica la provee la transacción. Este
   * helper se conserva para compatibilidad pero no debe usarse para update
   * directo de table fuera de FSM (evitar bypass de auditoría).
   */
  private static async revertSeatingAndTable(
    tableId: string,
    waitlistId: string,
    ...
    await fsmService.attemptTransition({
      ...
      source: SignalSource.SYSTEM_TIMEOUT,
      trigger: 'Rollback de asignación por fallo en pre-pedido',
      staffUserId,
      expectedCurrentState: TableFSMState.OCCUPIED_NO_ORDER
    }).catch(err => {
      console.error('Fallo al revertir mesa en FSM:', err);
    });
  }
}
