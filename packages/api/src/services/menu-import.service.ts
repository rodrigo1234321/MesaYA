...
export class MenuImportService {
  ...
  static async importMenu(
    ...
    for (let index = 0; index < items.length; index++) {
      ...
      if (!Number.isFinite(rawPrice) || rawPrice < 0 || rawPrice > 100_000_000) {
        throw new MenuImportError(`El precio del plato "${name}" debe ser un número finito no negativo menor a 100.000.000`);
      }
